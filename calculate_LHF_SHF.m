...
EndFile = '090000-JPL-L4_GHRSST-SSTfnd-MUR-GLOB-v02.0-fv04.1_rec.nc';

AnalysedFile_Ix = [YearFile,MonthFile,DayFile,EndFile];
...
StartFile = 'cmems_obs-wind_glo_phy_my_l4_0.125deg_PT1H_';
YearFile = sprintf('%04d', year(AnalysedDate));
MonthFile =  sprintf('%02d', month(AnalysedDate));
DayFile = sprintf('%02d', day(AnalysedDate));
HourFile = '08_R';
...
EndFile = 'T18_14_rec.nc';

AnalysedFile_Ix = [StartFile,YearFile,MonthFile,DayFile,HourFile,...
    YearFilePrev,MonthFilePrev,DayFilePrev,EndFile];
...
if isempty(AnalisedFile)
    HeatFlux(:,:,f) = NaN(xl,yl);
     [StartFile, DayFile, MonthFile, YearFile]
    continue;
end
...
% --------------------------------------------------------------------- %
% --------------------------- Input Files: ---------------------------- %
% --- Air Density, Dew Point Temperature and Air Temperature (ERA5) --- %
% --------------------------------------------------------------------- %
...
AnalysedFile_Ix = strjoin({StartFile, YearFile, MonthFile,DayFile, ...
    FormatFile}, '.');
...
DataFolder = "";  % Path to the folder containing air pressure data
FolderContents = dir(DataFolder); 
FileNames = cell(0); 
